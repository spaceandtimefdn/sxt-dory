...
fn test_soundness_binding_property() {
    ...
    let commitment1 = commit::<ArkBn254Pairing, OptimizedMsmG1>(&coeffs1, 0, sigma, &prover_setup);
...
fn test_soundness_offset_manipulation() {
    ...
    let commitment1 = commit::<ArkBn254Pairing, OptimizedMsmG1>(&coeffs, 0, sigma, &prover_setup);
...
fn test_soundness_commitment_consistency() {
    ...
    let commitment1 = commit::<ArkBn254Pairing, OptimizedMsmG1>(&coeffs, 0, sigma, &prover_setup);
    let commitment2 = commit::<ArkBn254Pairing, OptimizedMsmG1>(&coeffs, 0, sigma, &prover_setup);
    ...
    assert_eq!(
        ...
        "Same polynomial should produce same evaluation"
    );
...
fn test_soundness_degree_bound() {
    ...
    // Try to claim wrong evaluation
    let wrong_evaluation = evaluation2 + Fr::rand(&mut rng);
    let result = verify::<ArkBn254Pairing, _, OptimizedMsmG1, OptimizedMsmG2, DummyMsm<_>>(
        commitment,
        wrong_evaluation,
        &point,
