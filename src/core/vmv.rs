  ...
  <E::G1 as Group>::Scalar: Field,
{
  ...
  // Only require divisibility when expanding (ν > σ). When ν <= σ we keep base_len and do not shrink.
  debug_assert!(
      base_len == 0 || target_len < base_len || target_len % base_len == 0,
      "target 2^ν must be multiple of 2^σ"
  );
...
pub(crate) fn build_dory_prover_state<
  E: Pairing,
